...
const PassphraseList: FC<IPassphraseListProps> = ({
  passphrases
}) => {
  const [searchTerm, setSearchTerm] = useState<string>("")

  const filteredPassphrases = passphrases.filter((
    passphrase: ListableDatabaseEntry
  ) => {
      ...
      {searchTerm.length > 0 &&
        ...
        >
          {filteredPassphrases.length > 0
            ? <>
              {
                filteredPassphrases.length
              } result{
                filteredPassphrases.length > 1 ? "s" : ""
              } found</>
            : "No results found"
          }
        </motion.p>
    ...
    >
      {filteredPassphrases
        .sort((leftHand, rightHand) =>
          leftHand.platform.localeCompare(rightHand.platform)
        ).map((passphrase) =>
          <PassphraseCard
            key={passphrase.id}
            {...passphrase}
