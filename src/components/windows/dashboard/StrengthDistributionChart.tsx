...
const StrengthDistributionChart: FC<IStrengthDistributionChartProps> = ({ strengths }) => {
  const data = Array.from({ length: 11 }, (_, index) => ({
    ...
    value: Object.keys(strengths).filter(key => strengths[key] === index + LENGTH_PENALTY).length
  }))

  const colors = Array.from({ length: 11 }, (_, index) => Strength.color(index + LENGTH_PENALTY))
