...
const SelectableCSVRow: FC<ISelectableCSVRowProps> = ({
  ...
  return <tr>
    <td
      ...
      onKeyDown={(event) => ["Enter", " "].includes(event.key) && toggleSelection()}
      className={classNames({
        "bg-tuatara-50 dark:bg-tuatara-800 cursor-pointer w-8 h-8 border-2 transition-all": true,
    ...
    >
      <div className={classNames({
        "border-l-2 border-b-2 border-leaf-500 ml-1.5 -rotate-45 -mt-1 w-4 h-2 transition-all": true,
      ...
      entry.map((value, index) =>
        ...
        >
          <input
            ...
            className={classNames({
              "bg-transparent w-full p-1 min-w-0 transition-all": true,
